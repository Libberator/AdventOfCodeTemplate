...
public struct Vector3Int(int x, int y, int z)
{
    ...
    {
        Clamp(min.X, max.X, min.Y, max.Y, min.Z, max.Z);
    }

    /// <summary>Computes the Euclidian distance between <paramref name="other" /> and this vector.</summary>
    public readonly double DistanceEuclidianTo(Vector3Int other)
    {
    ...
    {
        return Math.Max(Math.Max(Math.Abs(other.X - X), Math.Abs(other.Y - Y)), Math.Abs(other.Z - Z));
    }
    ...
    public readonly int DistanceSquaredTo(Vector3Int other)
    {
    ...
    {
        return DistanceManhattanTo(other) == 2 && DistanceChebyshevTo(other) == 1;
    }
    ...
    {
        return other != this && (other.X == X || other.Y == Y || other.Z == Z);
    }
    ...
    */
    ...
    public void Subtract(Vector3Int other)
    {
    ...
    {
        return Clamp(value, min.X, max.X, min.Y, max.Y, min.Z, max.Z);
    }

    /// <summary>Restricts a vector between a minimum and a maximum value.</summary>
    public static Vector3Int Clamp(Vector3Int value, int minX, int maxX, int minY, int maxY, int minZ, int maxZ)
    {
        return new Vector3Int(Math.Clamp(value.X, minX, maxX), Math.Clamp(value.Y, minY, maxY),
    ...
    public static double DistanceEuclidian(Vector3Int a, Vector3Int b)
    {
    ...
    {
        return Math.Max(Math.Max(Math.Abs(b.X - a.X), Math.Abs(b.Y - a.Y)), Math.Abs(b.Z - a.Z));
    }
    ...
    */
    ...
    {
        return new Vector3Int(value.X / divisor, value.Y / divisor, value.Z / divisor);
    }
