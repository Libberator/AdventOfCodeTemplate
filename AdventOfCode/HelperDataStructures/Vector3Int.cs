...
public struct Vector3Int(int x, int y, int z) : IEquatable<Vector3Int>, IFormattable
{
    ...
    public int this[int index]
    {
        readonly get => index switch
        {
            ...
            1 => Y,
            2 => Z,
            _ => throw new ArgumentOutOfRangeException($"{index} is not a valid index for Vector3Int")
        };
        set
        {
            switch (index)
            {
                ...
                default: throw new ArgumentOutOfRangeException($"{index} is not a valid index for Vector3Int");
            }
        }
    }
    ...
    public static readonly Vector3Int[] AllDirections = [Right, Up, Forward, Left, Down, Backward];
    ...
    public void Clamp(int minX, int maxX, int minY, int maxY, int minZ, int maxZ)
    {
    ...
    /// <summary>Computes the Euclidean distance between <paramref name="other" /> and this vector.</summary>
    public readonly double DistanceEuclideanTo(Vector3Int other) => Math.Sqrt(DistanceSquaredTo(other));

    /// <summary>
    ///     Computes the Chebyshev distance. In 2d, this is known as chessboard distance - the amount of moves a king
    ///     would take to get from a to b.
    /// </summary>
    public readonly int DistanceChebyshevTo(Vector3Int other) =>
    ...
    public readonly bool IsDiagonalTo(Vector3Int other) =>
        DistanceManhattanTo(other) == 2 && DistanceChebyshevTo(other) == 1;

    /// <summary>Lies along at least 1 same axis at any distance, but not the same position</summary>
    public readonly bool IsLateralTo(Vector3Int other) =>
        other != this && (other.X == X || other.Y == Y || other.Z == Z);

    /* TODO: work out the math on this for 3D
    /// <summary>Returns a value that indicates if this vector and <paramref name="other"/> are parallel and neither are Vector2Int.Zero</summary>
    ...
    */
    ...
    public void MultiplyBy(Vector3Int scalars)
    {
        X *= scalars.X;
        Y *= scalars.Y;
        Z *= scalars.Z;
    ...
    public static Vector3Int Clamp(Vector3Int value, Vector3Int min, Vector3Int max) =>
        Clamp(value, min.X, max.X, min.Y, max.Y, min.Z, max.Z);

    /// <summary>Restricts a vector between a minimum and a maximum value.</summary>
    public static Vector3Int Clamp(Vector3Int value, int minX, int maxX, int minY, int maxY, int minZ, int maxZ) =>
    ...
    public static double DistanceEuclidean(Vector3Int a, Vector3Int b) => Math.Sqrt(DistanceSquared(a, b));

    /// <summary>
    ///     Computes the Chebyshev distance, also known as chessboard distance - the amount of moves a king would take to
    ///     get from a to b.
    /// </summary>
    public static int DistanceChebyshev(Vector3Int a, Vector3Int b) =>
    ...
    */
    /// <summary>
    ///     Returns all points in a box/plane/line between the corner points <paramref name="from" /> and
    ///     <paramref name="to" />, inclusive.
    /// </summary>
    ...
    public static IEnumerable<Vector3Int> GetAllPointsBetween(int minX, int maxX, int minY, int maxY, int minZ,
        int maxZ)
    {
        for (var x = minX; x <= maxX; x++)
        for (var y = minY; y <= maxY; y++)
    ...
    /// <summary>
    ///     Returns a direct "king-walk" path with diagonals first, <paramref name="from" /> and <paramref name="to" />
    ///     inclusive.
    /// </summary>
    ...
    public static Vector3Int Map(Vector3Int fromMin, Vector3Int fromMax, Vector3Int toMin, Vector3Int toMax,
        Vector3Int value) =>
    ...
    /// <summary>Returns a value that indicates whether this instance and another vector are equal.</summary>
    public readonly override bool Equals([NotNullWhen(true)] object? obj) => obj is Vector3Int other && Equals(other);
    ...
    public static Vector3Int operator /(Vector3Int value, int divisor) =>
