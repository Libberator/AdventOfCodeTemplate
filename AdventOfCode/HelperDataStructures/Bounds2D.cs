...
public struct Bounds2D(int xMin, int xMax, int yMin, int yMax)
{
    ...
    /// <summary>Returns a value that is the distance from the closest point on the Bounds' border.</summary>
    public readonly int DistanceFromBorder(int x, int y) => new Vector2Int(x, y).DistanceManhattanTo(ClosestPointOnBorder(x, y));
    ...
    public readonly bool Overlaps(Bounds2D other) => XMin <= other.XMax && other.XMin <= XMax && YMin <= other.YMax && other.YMin <= YMax;
