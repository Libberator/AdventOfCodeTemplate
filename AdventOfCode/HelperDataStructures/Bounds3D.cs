...
public struct Bounds3D(int xMin, int xMax, int yMin, int yMax, int zMin, int zMax)
{
    ...
    public Bounds3D(Vector3Int center, Vector3Int extents) :
        this(center.X - extents.X, center.X + extents.X, center.Y - extents.Y, center.Y + extents.Y, center.Z - extents.Z, center.Z + extents.Z)
    { }
    ...
    /// <summary>Returns a value that is the distance from the closest point on the Bounds' border.</summary>
    public readonly int DistanceFromBorder(int x, int y, int z) => new Vector3Int(x, y, z).DistanceManhattanTo(ClosestPointOnBorder(x, y, z));
    ...
    /// <summary>Returns a value to indicate the position is directly on the edge of the Bounds.</summary>
    public readonly bool IsOnEdge(int x, int y, int z) => Contains(x, y, z) && (x == XMin || x == XMax || y == XMin || y == YMax || z == ZMin || z == ZMax);
    /// <summary>Returns a value to indicate if another bounding box intersects or shares an edge with this bounding box.</summary>
    public readonly bool Overlaps(Bounds3D other) => XMin <= other.XMax && other.XMin <= XMax && YMin <= other.YMax && other.YMin <= YMax && ZMin <= other.ZMax && other.ZMin <= ZMax;
