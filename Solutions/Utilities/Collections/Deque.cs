...
public class Deque<T> : IList<T>, IReadOnlyList<T>, IList
{
    ...
    public Deque(IEnumerable<T> collection)
    {
        ...
        var source = collection.ReifyCollection();
...
internal static class DequeHelper
{
    /// <summary>
    ///     Turn an abstract type into a real collection type
    /// </summary>
    public static IReadOnlyCollection<T> ReifyCollection<T>(this IEnumerable<T> source)
    {
        switch (source)
        {
            case null:
        ...
        return enumerated.Length == 0 ? new List<T>() : new List<T>(enumerated);
    }
